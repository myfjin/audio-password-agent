import XCTest
@testable import audiopasswdmanagerCore

final class audiopasswdmanagerTests: XCTestCase {
    ...
    func testInitCreatesVaultConfig() throws {
        _ = try audiopasswdmanager(masterPassword: "pw", vaultDirectory: vaultDir)
    ...
    func testReinitWithSamePasswordSucceeds() throws {
        ...
        let agent1 = try audiopasswdmanager(masterPassword: "pw", vaultDirectory: vaultDir)
        ...
        let agent2 = try audiopasswdmanager(masterPassword: "pw", vaultDirectory: vaultDir)
    ...
    func testStoreAndRetrieveRoundtrip() throws {
        let output = vaultDir.appendingPathComponent("out.wav")
        let agent  = try audiopasswdmanager(masterPassword: "master123", vaultDirectory: vaultDir)
    ...
    func testRoundtripPreservesMetadata() throws {
        let output = vaultDir.appendingPathComponent("out.wav")
        let agent  = try audiopasswdmanager(masterPassword: "pw", vaultDirectory: vaultDir)
    ...
    func testRetrieveWithWrongMasterPasswordThrows() throws {
        let output = vaultDir.appendingPathComponent("out.wav")
        let agent1 = try audiopasswdmanager(masterPassword: "correct", vaultDirectory: vaultDir)
        try agent1.storeCredential(
        ...
        let agent2 = try audiopasswdmanager(masterPassword: "wrong", vaultDirectory: altVaultDir)
