...
struct EditorPanelView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingEdit) {
            if let clip { EditCredentialSheet(clip: clip).environmentObject(vm) }
        }
        .confirmationDialog("Delete \"\(clip?.service ?? "")\"?",
                            isPresented: $showingDelete, titleVisibility: .visible) {
            Button("Delete", role: .destructive) {
                if let path = clip?.wavFile { vm.deleteCredential(wavPath: path) }
            }
        } message: {
        ...
        HStack {
            ...
            Button { showingDelete = true } label: {
                Image(systemName: "trash")
                    .font(.system(size: 10, weight: .medium))
                    .foregroundStyle(Color(hex: "FF5C5C").opacity(0.7))
            }
            .buttonStyle(.plain)
            .padding(.trailing, 8)
